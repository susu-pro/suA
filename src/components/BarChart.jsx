...
const BarChart = ({ data, selectedCity, selectedYear, onYearChange }) => {
  ...
  const drawChart = () => {
    ...
    const width = 500;
    const height = 400;
    const margin = { top: 50, right: 120, bottom: 100, left: 60 };
    ...
    const yScale = d3.scaleLinear()
      .domain([0, d3.max(cityData, d => d.count) * 1.2])
      .range([innerHeight, 0]);
    ...
    cityData.forEach(d => {
      const gradient = defs.append("linearGradient")
        .attr("id", `bar-gradient-${d.star}-${selectedYear}`)
      ...
      gradient.append("stop")
        .attr("offset", "0%")
        .attr("stop-color", d3.color(colorScale(d.star)).brighter(0.3))
    ...
    const bars = g.selectAll(".bar")
      ...
      .attr("height", 0)
      .attr("fill", d => `url(#bar-gradient-${d.star}-${selectedYear})`)
      .attr("rx", 4)
      .attr("ry", 4)
    ...
    svg.append("text")
      ...
      .attr("y", 25)
      .attr("text-anchor", "middle")
      .text(`${selectedCity}酒店数量分布 (${selectedYear}年)`)
